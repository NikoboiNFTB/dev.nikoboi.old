...
// @version      3.2
// @description  Adds a native-style “WATCH” button on IMDb movie or TV pages, redirecting directly to 111movies with the correct title or episode link using MutationObserver for instant injection on the popularity container.
// @author       Nikoboi
...
(function () {
    ...
    function injectButton(target) {
        ...
        const isTVPage = !!document.querySelector('[aria-label="View Popular TV Shows"]');
        const isMoviePage = !!document.querySelector('[aria-label="View Popular Movies"]');

        if (isTVPage) {
            const tvGroup = createNativeButtonGroup('WATCH');
            ...
            console.log('✅ TV button added (MutationObserver on popularity container).');
        } else if (isMoviePage) {
            const movieGroup = createNativeButtonGroup('WATCH');
            const goBtn = createGoButton('#28a745');
            ...
            console.log('✅ MOVIE button added (MutationObserver on popularity container).');
        } else {
            console.warn('⚠️ IMDb-RePo: Could not determine title type.');
            return false;
        }
    ...
    // Observe only the popularity container instead of the whole body
    const targetNode = document.querySelector('[data-testid="hero-rating-bar__popularity"]');

    if (targetNode) {
        // Already exists, inject immediately
        const mainTarget = document.querySelector('.sc-8e956c5c-1.dzAXeE');
        if (mainTarget) injectButton(mainTarget);
    } else {
        // Wait for it via MutationObserver
        const observer = new MutationObserver((mutations, obs) => {
            const popularityNode = document.querySelector('[data-testid="hero-rating-bar__popularity"]');
            const mainTarget = document.querySelector('.sc-8e956c5c-1.dzAXeE');
            if (popularityNode && mainTarget) {
                injectButton(mainTarget);
